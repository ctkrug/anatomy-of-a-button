...
export const STAGES = [
  ...
  },
  {
    ...
    body: "Before anything is drawn, the parser turns your markup into a tree. The button isn't a picture yet; it's an object with a parent, siblings, and a position in the document. Every style rule that follows is matched against this structure, which is why a selector like .toolbar > button can find it at all.",
  },
  {
    id: "box",
    label: "02 · layout",
    ...
    body: "Layout resolves the node into four nested rectangles: the content, the padding around it, the border around that, and the margin holding everything else away. The button you see is only the border box. The space it claims on the page is bigger, and this arithmetic, not the paint, decides where every other element lands.",
  },
  {
    id: "paint",
    label: "03 · paint",
    ...
    body: "Painting doesn't produce a button. It produces an ordered list of drawing operations: shadow, background, border, then text. Each fills the same rectangle and is drawn on top of the last. The order is fixed by the spec, which is why a background can never cover its own text.",
  },
  {
    id: "composite",
    label: "04 · composite",
  ...
  },
  {
    ...
    body: "Tree, boxes, paint order, layers, composite: all of it, every frame, for one button that says Click me. It should look a little less boring now.",
  },
];
